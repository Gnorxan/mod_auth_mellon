...
/* The default setting for setting MELLON_SAML_ASSERTION
 */
static const int default_dump_saml_assertion = 0;
...
const command_rec auth_mellon_commands[] = {
	...
	AP_INIT_FLAG(
		"MellonSamlAssertionDump",
		ap_set_flag_slot,
		(void *)APR_OFFSETOF(am_dir_cfg_rec, dump_saml_assertion),
		OR_AUTHCFG,
		"Dump SAML authentication assertion in environment. Default is off"
		),
    AP_INIT_RAW_ARGS(
        "MellonRequire",
        am_set_require_slot,
...
void *auth_mellon_dir_config(apr_pool_t *p, char *d)
{
    ...
    dir->dump_saml_assertion = default_dump_saml_assertion;
...
void *auth_mellon_dir_merge(apr_pool_t *p, void *base, void *add)
{
    ...
    new_cfg->dump_saml_assertion =
        (add_cfg->dump_saml_assertion != default_dump_saml_assertion ?
         add_cfg->dump_saml_assertion :
         base_cfg->dump_saml_assertion);
